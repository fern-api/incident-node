...
```sh
npm i -s @fern-api/incidentio
```

## Reference

A full reference for this library is available [here](https://github.com/fern-api/incident-node/blob/HEAD/./reference.md).
...
```typescript
const response = await client.utilitiesV1.openapi(...);
...
```typescript
const { data, rawResponse } = await client.customFieldOptionsV1.create(...).withRawResponse();
